...
void read_turbo_limit()
{
	...
	read_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT, &turbo_limit);
...
{
	uint64_t turbo_limit;
	read_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT_CORES, &turbo_limit);
	return turbo_limit;// & 0xFF;
}
...
void set_turbo_limit(unsigned long limit)
{
	...
	limit &= 0xFF;
	ratio_limit = 0x0ul | (limit) | ((limit - 2) << 8) | ((limit - 3) << 16) 
		| ((limit - 6) << 24);
	write_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT, ratio_limit);
	uint64_t core_limit;
	core_limit = 0x0ul | (2ul) | (4ul << 8) | (8ul << 16) | (12ul << 24);
	write_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT_CORES, core_limit);
}

void set_all_turbo_limit(uint64_t limit)
{
	write_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT, limit);
}

void set_all_turbo_limit1(uint64_t limit)
{
	write_msr_by_coord(1, 0, 0, MSR_TURBO_RATIO_LIMIT_CORES, limit);
}
...
void set_perf(const unsigned freq, const unsigned numcores)
{
	...
	if (perf_ctl == 0x0ul)
	{
		read_msr_by_coord(1, 0, 0, IA32_PERF_CTL, &perf_ctl);
	}
...
inline void disable_rapl()
{
	...
	read_msr_by_coord(1, 0, 0, MSR_PKG_POWER_LIMIT, &rapl_disable);
	rapl_disable &= 0xFFFFFFFFFFFE7FFFul;
	rapl_disable |= 0x1FFFul;
	write_msr_by_coord(1, 0, 0, MSR_PKG_POWER_LIMIT, rapl_disable);
}
...
{
	uint64_t prapl;
	read_msr_by_coord(1, 0, 0, 0x64D, &prapl);
	fprintf(stderr, "platform rapl is %lx\n", prapl);
	read_msr_by_coord(1, 0, 0, 0x65C, &prapl);
...
{
	uint64_t hwpe;
	read_msr_by_coord(1, 0, 0, 0x770, &hwpe);
...
{
	uint64_t plimit;
	read_msr_by_coord(1, 0, 0, 0x64F, &plimit);
